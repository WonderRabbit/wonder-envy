...
Fish에서 `dev`를 실행하면 nvm·pyenv·SDKMAN을 초기화한 Zsh에 들어간다. 아래 버전 변경 명령은 프로젝트 요구가 있을 때 실행한다. 인증값을 명령 예시나 공유 파일에 직접 기록하지 않는다.
...
| Codex | Zsh에서 `codex`, 버전 `codex --version`, 인증 확인 `codex login status` | [공식 CLI 문서](https://learn.chatgpt.com/docs/codex/cli) |
| Claude Code | `claude`, 최초 인증 `claude auth login`, 플러그인 확인 `claude plugin list` | [공식 문서](https://code.claude.com/docs/en/overview) |
| OpenCode | Zsh에서 `opencode`; 앱 내 `/connect`로 공급자 연결 | [공식 문서](https://opencode.ai/docs/) |
